...
Not all command line arguments are strings, but for some reason, our CLI parsers force us to use strings everywhere. 🤔 `@watts-ai/cli.ts` is a fully-fledged command line argument parser, influenced by Rust's [`clap`](https://github.com/clap-rs/clap) and [`structopt`](https://github.com/TeXitoi/structopt):
...
```ts
import { command, run, string, number, positional, option } from '@watts-ai/cli.ts';
...
What if we had a way to get a `Stream` out of the parser, instead of a plain string? This is where `@watts-ai/cli.ts` gets its power from, custom type decoding:
